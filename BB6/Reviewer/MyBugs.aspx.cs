...
namespace BB6.Reviewer
{
    public partial class MyBugs : System.Web.UI.Page
    {
        ...
        private void displayBugs()
        {
            ...
            cmd.CommandText = "SELECT * FROM BugDetails WHERE status = 'Pending Review'";
            cmd.Connection = conn;
        ...
        protected void btnSearch_Click(object sender, EventArgs e)
        {
            ...
            {
                Repeater1.DataSource = bc.getBugsByAssigneeReviewer(txtSearch.Text, status);
                lblError.Text = "";
            }
            else if (ddlSearchType.SelectedValue == "Keywords")
            {
                Repeater1.DataSource = bc.getBugsByKeywordsReviewer(txtSearch.Text, status);
                lblError.Text = "";
            }
            else if (ddlSearchType.SelectedValue == "Title")
            {
                Repeater1.DataSource = bc.getBugsByTitleReviewer(txtSearch.Text, status);
                lblError.Text = "";
            }
